...
class MalloryTest(tornado.testing.AsyncTestCase):
    ...
    def test_a_200_response_on_hitting_the_root_url(self):
        self.http_client.fetch(self.get_url("/"), self.stop, ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
        self.assertEqual(200, response.code)

    def test_path_is_proxied(self):
        self.http_client.fetch(self.get_url("/the/path/to/hit"), self.stop, ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
    ...
    def test_query_string_is_not_logged(self):
        ...
        self.http_client.fetch(self.get_url("/path?queryparam=value"), self.stop, ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
    ...
    def test_query_params_are_proxied(self):
        self.http_client.fetch(self.get_url("/path?param1=value1&param2=value2"), self.stop, ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
    ...
    def test_http_status_code_is_returned_in_the_response(self):
        self.http_client.fetch(self.get_url("/http_status/201"), self.stop, ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
        self.assertEqual(201, response.code)

    def test_http_headers_are_passed_along(self):
        self.http_client.fetch(self.get_url("/"), self.stop, ca_certs = "test/ssl/mallory/server.crt", headers = { "X-Something": "bar" })
        response = self.wait()
    ...
    def test_response_headers_are_returned(self):
        self.http_client.fetch(self.get_url("/http_header/X-CustomHeader/header-value"), self.stop, ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
        self.assertEqual('header-value', response.headers['X-CustomHeader'])

    def test_post_body_is_passed_along(self):
        self.http_client.fetch(self.get_url("/http_header/X-CustomHeader/header-value"), self.stop, method = "POST", body = "the post body", ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
    ...
    def test_put(self):
        self.http_client.fetch(self.get_url("/"), self.stop, method = "PUT", body = "the put body", ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
    ...
    def test_delete(self):
        self.http_client.fetch(self.get_url("/"), self.stop, method = "DELETE", ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
        self.assertTrue(response.body.find("METHOD: DELETE") >= 0, response.body)

    def test_head(self):
        self.http_client.fetch(self.get_url("/"), self.stop, method = "HEAD", ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
    ...
    def test_error_handling(self):
        ...
        self.http_client.fetch("https://127.0.0.1:10002/", self.stop, method = "HEAD", ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
    ...
    def test_timeout(self):
        self.http_client.fetch(self.get_url("/timeout"), self.stop, request_timeout=3, ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
        self.assertEqual(502, response.code)
        self.assertEqual(socket.gethostname(), response.headers['X-Proxy-Server'])

    def test_gzipped_response(self):
        self.http_client.fetch(self.get_url("/gzip"), self.stop, request_timeout=3, ca_certs = "test/ssl/mallory/server.crt")
        response = self.wait()
        self.assertEqual(200, response.code)
